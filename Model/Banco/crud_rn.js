...
export default class ItemDatabase {
                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM Item LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS Item (id INTEGER PRIMARY KEY AUTOINCREMENT, descricao varchar(30), data datetime, prioridades INTEGER, marcado 0)');
                        }).then(() => {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM Item', []).then(([tx, results]) => {
                        ...
                        for (let i = 0; i < len; i++) {
                            ...
                            const { id, descricao, data, prioridades, marcado } = row;
                            lista.push({ id, descricao, data, prioridades, marcado });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('INSERT INTO Item (descricao, data, prioridades, marcado) VALUES (?, ?, ?, ?)', [item.descricao, item.data, item.prioridades, item.marcado]).then(([tx, results]) => {
                        console.log('O numero do banco de dados é::::::::' + item.prioridades)
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
    ...
    Atualizar(marcado, id) {   //**** CRUD => UPDATE - aqui a tabela é atualizada *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para atualizar um registro no banco
            
                    tx.executeSql('UPDATE Item SET marcado = ? WHERE id = ?', [marcado, id]).then(([tx, results]) => {
                        console.log('O numero do banco de dados é::::::::' + marcado)
                        resolve(results);
                    });
                }).then((result) => {
                    console.log('O numero do banco de dados é::::::::' + marcado)
